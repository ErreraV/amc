...
class RealtimeAnalyzer:
    ...
    def get_real_time_stats(self, window_size=50) -> Dict:
        ...
        return {
            ...
            'prediction_accuracy': float(sum(1 for m in recent if m.prediction_error <= 3.0) / len(recent)),
            'avg_latency_ms': float(np.mean([m.decision_latency_ms for m in recent])),
            'avg_throughput': float(np.mean([m.throughput for m in recent])),
            'avg_bler': float(np.mean([m.bler for m in recent])),
            'avg_ber': float(np.mean([m.ber if m.ber > 0 else 1e-6 for m in recent])),
            'success_rate': float(sum(1 for m in recent if m.success) / len(recent)),
            'last_sample_time': recent[-1].timestamp if recent else 0,
            'decision_method_distribution': {
                'gan_preselected': sum(1 for m in recent if 'preselected' in m.decision_method),
                'rl_fallback': sum(1 for m in recent if 'rl_agent' in m.decision_method)
            }
        }
    ...
    def get_latency_histograms(self) -> Dict:
        ...
        return {
            'decision_latency': {
                'histogram': self.decision_latency_hist.get_histogram(),
                'stats': self.decision_latency_hist.get_stats()
            },
            'sionna_latency': {
                'histogram': self.sionna_latency_hist.get_histogram(),
                'stats': self.sionna_latency_hist.get_stats()
            },
            'gan_latency': {
                'histogram': self.gan_latency_hist.get_histogram(),
                'stats': self.gan_latency_hist.get_stats()
            }
        }
    ...
    def get_latency_breakdown(self) -> Dict:
        ...
        decision_logic_times = [m.decision_latency_ms - m.sionna_latency_ms for m in metrics]
        gan_times = [m.gan_latency_ms for m in metrics if m.gan_latency_ms > 0]
        sionna_times = [m.sionna_latency_ms for m in metrics]
        ...
        return {
            ...
            'sionna_simulation': {
                ...
                'max_ms': float(np.max(sionna_times))
            }
        }
